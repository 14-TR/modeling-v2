...
from config import grid_size, start_res, start_ttd, ttd_rate, res_lose_rate, inf_rate, w, h, \
    vi, vj, z, num_humans, num_zombies, epochs, days, num_resources, loser_survival_rate, loser_death_rate
...
class Simulation:
    ...
    def __init__(self, humans=num_humans, zombies=num_zombies, e=epochs, d=days):
        self.grid = Grid(grid_size=grid_size, num_resources=num_resources)
        self.network_manager = NetworkManager()
        self.humans = [Human(network_manager=self.network_manager) for _ in range(humans)]
    ...
    def process_interactions(self, entity):
        ...
        for other in self.humans + self.zombies:
            if entity != other and self.grid.are_adjacent(entity, other) and entity.is_active and other.is_active:
                # Both entities are active
...
class Entity:
    grid = Grid(grid_size=grid_size, num_resources=num_resources)
...
class Human(Entity):
    def __init__(self, res=start_res, network_manager=None):
        super().__init__(entity_type='H', network_manager=network_manager)
        self.att['res'] = res
        self.is_h = True
        self.is_z = False
        self.is_active = True
    ...
    def distribute_resources(self):
        if self.grp:
            ...
            members = [entities[m_id] for m_id in self.grp.keys() if
                       m_id in entities and self.is_adjacent(entities[m_id])]
            if members:
